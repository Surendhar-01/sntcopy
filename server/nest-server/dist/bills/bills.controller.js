...
let BillsController = class BillsController {
    ...
    async remove(id) {
        const result = await this.billsService.remove(Number(id));
        if (!result) {
            throw new common_1.HttpException('Bill not found', common_1.HttpStatus.NOT_FOUND);
        }
        return result;
    }
    clearAll() {
        return this.billsService.clearAll();
    }
};
...
__decorate([
    ...
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], BillsController.prototype, "clearAll", null);
